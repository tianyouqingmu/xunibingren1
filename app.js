(function () {
  ...
  function getScfProxyUrl(settings = null) {
    const currentSettings = settings || readSettingsForm();
    return normalizeProxyUrl(currentSettings.scfProxyUrl);
  }

  function proxyEndpoint(path, settings = null) {
    const base = getScfProxyUrl(settings);
    if (!base) return "";
    return `${base}/${String(path || "").replace(/^\/+/, "")}`;
  }

  function updateBackendControls() {
    const verifyButton = $("verifyTencent");
    if (!verifyButton) return;
    const hasProxy = Boolean(getScfProxyUrl());
    verifyButton.disabled = !backendAvailable && !hasProxy;
    verifyButton.textContent = backendAvailable || hasProxy ? "验证云服务连接" : "公开版需填写云函数代理地址";
  }
  ...
  function enterStaticMode() {
    ...
    if (getScfProxyUrl()) {
      setCloudStatus("公开网页模式：已配置云函数代理，将通过云函数调用 ASR/TTS/大模型");
    } else {
      setCloudStatus("公开网页模式：腾讯云 ASR/TTS 需要云函数代理；当前使用浏览器语音/本地朗读");
    }
  }
  ...
  function renderVerification(verification) {
    ...
    if (tts.ok && tts.audioBase64) playAudioBase64(tts.audioBase64, tts.contentType || "audio/wav");
    else if (tts.ok && tts.audioUrl) playAudioUrl(tts.audioUrl);
  }
  ...
  async function saveBackendSettings(settings) {
    if (!backendAvailable) {
      enterStaticMode();
      if (getScfProxyUrl(settings)) {
        setVerifyStatus("配置已保存到本机浏览器。公开版将通过云函数代理调用云服务，密钥保存在云函数环境变量中。", null);
      } else {
        setVerifyStatus("配置已保存到本机浏览器。GitHub Pages 是纯静态网页，不能保存或验证腾讯云 SecretKey。", null);
      }
  ...
  async function verifyTencentConnection() {
    if (!backendAvailable) {
      const endpoint = proxyEndpoint("verify");
      if (endpoint) {
        setVerifyStatus("正在通过云函数验证云服务连接，请稍等...", null);
        ...
        if (data.ok === false) throw new Error(data.error || "云函数验证未全部通过，请查看 ASR/TTS 提示。");
        return data.verification;
      }
      enterStaticMode();
      setVerifyStatus("公开版没有后端，无法验证腾讯云连接。需要腾讯云 ASR/TTS 时，请部署后端代理或在本机运行 start_web_preview.bat。", false);
      return null;
    }
    ...
    if (data.verification) renderVerification(data.verification);
    if (data.ok === false) throw new Error(data.error || "本地后端验证未全部通过，请查看 ASR/TTS 提示。");
    return data.verification;
  }
  ...
  async function requestLlm(question, settings) {
    ...
    if (settings.llmMode === "proxy") {
      const endpoint = normalizeProxyUrl(settings.llmProxyUrl) || proxyEndpoint("llm", settings);
      if (!endpoint) throw new Error("请先填写云函数代理地址或大模型代理地址。");
  ...
  async function speak(text, question = "") {
    if (!text) return;
    const settings = readSettingsForm();
    const scfTtsEndpoint = proxyEndpoint("tts", settings);
    if (!backendAvailable && !scfTtsEndpoint) {
      setStatus("公开网页模式：正在使用浏览器朗读。");
      fallbackBrowserSpeak(text);
      return;
    }
    try {
      setStatus(backendAvailable ? "正在调用腾讯云实时 TTS..." : "正在通过云函数调用腾讯云实时 TTS...");
      const response = await fetch(backendAvailable ? "/api/tts" : scfTtsEndpoint, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  function startBrowserVoice() {
    ...
    if (!Recognition) {
      throw new Error("公开网页模式下，当前浏览器不支持语音识别，请用文字输入。");
    }
    ...
    browserRecognition.onstart = () => {
      ...
      $("voiceButton").setAttribute("title", "结束浏览器语音识别");
      $("voiceButton").setAttribute("aria-label", "结束浏览器语音识别");
      setStatus("正在使用浏览器语音识别，再按一次麦克风可结束。");
    };
  ...
  async function stopTencentRecording() {
    const settings = readSettingsForm();
    const asrEndpoint = backendAvailable ? "/api/asr" : proxyEndpoint("asr", settings);
    recording = false;
    $("voiceButton").classList.remove("active");
    ...
    try {
      ...
      setStatus(backendAvailable ? "正在调用腾讯云一句话识别..." : "正在通过云函数调用腾讯云一句话识别...");
      const response = await fetch(asrEndpoint, { method: "POST", body: form });
      const data = await response.json().catch(() => ({}));
      if (!response.ok || data.ok === false) throw new Error(data.error || `HTTP ${response.status}`);
  ...
  async function toggleTencentVoice() {
    try {
      const settings = readSettingsForm();
      if (!backendAvailable && !getScfProxyUrl(settings)) {
  ...
  $("scfProxyUrl").addEventListener("input", () => {
    updateBackendControls();
    if (!backendAvailable) enterStaticMode();
  });
  ...
  $("saveSettings").addEventListener("click", async () => {
    ...
    try {
      const result = await saveBackendSettings(settings);
      if (result && result.staticMode) {
        setStatus("公开版配置已保存到本机浏览器。腾讯云验证需要后端代理，本页面不会上传或保存 SecretKey。");
      } else {
  ...
  $("verifyTencent").addEventListener("click", async () => {
    try {
      const verification = await verifyTencentConnection();
      if (verification) setStatus("腾讯云连接验证完成。");
    } catch (error) {
